...
# Usage: runTest_2011.sh -TPD [usually]
# Be sure to set METAMAP_TEST and METAMAP_PROD just below the comments
# Set METAMAP_PROD here!!
METAMAP_PROD="metamap14 -V USAbase -Z 2014AA"
# and
# Set METAMAP_TEST here!!
METAMAP_TEST="/home/luizperciliano/metamap-portugues/metamap/public_mm/bin/metamap14"
...
run_MetaMap() {
   ...
   INFILE=$FILENAME.txt
   TMP_OUTFILE=$FILENAME.${PRODorDEVL}.OUT
   OUTFILE=$FILENAME.${PRODorDEVL}

   COMMAND="$PROGRAM $FLAGS $INFILE $TMP_OUTFILE"
   echo $COMMAND
   $COMMAND
#    echo "cat $INFILE | $PROGRAM $FLAGS > $TMP_OUTFILE 2>&1 "
#    cat $INFILE | $PROGRAM $FLAGS > $TMP_OUTFILE 2>&1 
...
#
#         FILENAME | PRODFLAGS | TESTFLAGS | TITLE | MODIFY
#
...
# METAMAP_PROD="SKRrun.14 /nfsvol/nls/II_Group_WorkArea/flang/specialist/SKR/src/a.out.Linux  -Z 2014AA -L 2014 --lexicon db"
METAMAP_PROD="metamap  -Z 2016AA -L 2016 --lexicon db"

# Set METAMAP_TEST here!!
# METAMAP_TEST is the new test version
METAMAP_TEST="metamap.FML  -Z 2016AA -L 2016 --lexicon db"
# METAMAP_TEST="metamap.FML  -Z 2014AB -L 2014 --lexicon db"
...
do
   ...
   # This call to split_line sets the five variables
   # FILENAME, PRODFLAGS, TESTFLAGS, TITLE and MODIFY
   # FILENAME | PRODFLAGS | TESTFLAGS | TITLE| MODIFY

   split_line "$LINE"

   # for ease of maintenance, allow TESTFLAGS to be empty,
   # in which case use PRODFLAGS as TESTFLAGS
   if [ "$TESTFLAGS" == "" ]
   then
       TESTFLAGS=$PRODFLAGS
   fi
   ...
   if [ $RUN_PROD -eq 1 ]
   then
      echo running run_MetaMap prod "$METAMAP_PROD" "$PRODFLAGS" $FILENAME $MODIFY
      run_MetaMap prod "$METAMAP_PROD" "$PRODFLAGS -E" $FILENAME $MODIFY
   fi

   if [ $RUN_TEST -eq 1 ]
   then
      echo running run_MetaMap test "$METAMAP_TEST" "$TESTFLAGS" $FILENAME $MODIFY
      run_MetaMap test "$METAMAP_TEST" "$TESTFLAGS -E" $FILENAME $MODIFY
   fi
...
JSON_format|--JSONf 2|--JSONf 2|JSON format|0|
JSON_noformat|--JSONn|--JSONn|JSON noformat|0|
num_break|--num_break|--num_break|num break ON|0|
num_break|||num break OFF|0|
no_nums|--no_nums ALL|--no_nums ALL|no nums ON|0|
no_nums|||no nums OFF|0|
conj|--conj|--conj|conj|0|
cascadeOFF|-J dsyn|-J dsyn|cascadeOFF|0|
cascadeON|-J dsyn --cascade|-J dsyn --cascade|cascadeON|0|
nomapOFF|-Z 2015AA|-Z 2015AA|nomapOFF|0|
nomapON|--nomap NoMapFile -Z 2015AA|--nomap NoMapFile -Z 2015AA|nomapON|0|
novarOFF|-Dv|-Dv|novarOFF|0|
novarON|-Dv --novar NoVarFile|-Dv --novar NoVarFile|novar|0|
allow_concept_gaps_compute_all_mappings_debug5|-gb --debug 5|-gb --debug 5|Concept Gaps, All Mappings, Debug5|0|
5zb|-zbI --debug 5|-zbI --debug 5|5zb Option|0|
...
NegEx_HR|--negex -Q 1|--negex -Q 1|NegEx HR Output|1|
...
allow_concept_gaps_compute_all_mappings_number_candidates|-gbnc|-gbnc|Concept Gaps, All Mappings, Number Candidates|0|
...
allow_overmatches|-ocmn|-ocmn|Overmatches|0|
...
show_candidates|-c|-c|Show Candidates|0|
hide_mappings|-mc|-mc|Hide Mappings, Show Candidates|0|
...
short_semantic_types|-s|-s|Short Semantic Types|0|
NLMSubSynNoFlagsStrict|||NLMSubSynNoFlagsStrict|0|
NLMSubSynNoFlagsRelaxed|-C|-C|NLMSubSynNoFlagsRelaxed|0|
ExcludeNLMSubSynStrict|-e NLMSubSyn|-e NLMSubSyn|ExcludeNLMSubSynStrict|0|
ExcludeNLMSubSynRelaxed|-e NLMSubSyn -C|-e NLMSubSyn -C|ExcludeNLMSubSynRelaxed|0|
RestrictToNLMSubSynStrict|-R NLMSubSyn|-R NLMSubSyn|RestrictToNLMSubSynStrict|0|
RestrictToNLMSubSynRelaxed|-R NLMSubSyn -C|-R NLMSubSyn -C|RestrictToNLMSubSynRelaxed|0|
AAsTest|--aas_only|--aas_only|AAs Test|0|
AAs1000|--aas_only --silent|--aas_only --silent|AAs1000|0|
EOF
